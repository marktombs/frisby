...
Frisby.prototype.expectEmbeddedResource = function(embeddedResourceName) {
  var self = this;

  this.current.expects.push(function() {
    var jsonBody = _jsonParse(self.current.response.body);
    if (!jsonBody.hasOwnProperty('_embedded')){
      throw new Error('Resource has no _embedded');
    }
    var embedded = jsonBody._embedded;
    if (!embedded.hasOwnProperty(embeddedResourceName)){
      throw new Error('Resource has no embedded resource called ' + embeddedResourceName);
    }
    if (embedded.hasOwnProperty(embeddedResourceName))
      expect(embedded[embeddedResourceName].length).toBeGreaterThan(0);

  });
  return this;
};

Frisby.prototype.expectSelfLink = function(){
  var self = this;

  this.current.expects.push(function(){
    var jsonBody = _jsonParse(self.current.response.body);
    if (!jsonBody.hasOwnProperty('_links')){
      throw new Error('Resource has no _links')
    }
    if (!jsonBody._links.self) {
      throw new Error('Resource has no self link');
    }
    expect(jsonBody._links.self.href).toBeTruthy();
